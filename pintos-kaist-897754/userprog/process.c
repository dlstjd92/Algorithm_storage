...
process_fork (const char *name, struct intr_frame *if_ UNUSED) {
	...
	// if (pid == 0)
	// {
	// 	struct thread *child_cur = thread_current();
	// 	list_push_back(&cur->child_list, &child_cur->child_elem);
	// }

	struct thread *child = get_child_process(pid);

	if (child->p_status == TID_ERROR) return -1;
...
process_exec (void *f_name) {
	...
	bool success;
	char* s[30]; //= "  String to  tokenize. "; 이중 포인터? 놉 포인터 배열
    char *token, *save_ptr;
	bool name_check = 1;
	int cnt = 0;
